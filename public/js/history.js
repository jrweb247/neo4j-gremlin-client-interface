(function () {
   ...
   function applyHistElem(hist_elem) {
      ...
      var pos = gremlin.editor.getCursorPosition();
      var prefix = "";
      var suffix = "\n";

      pos = gremlin.utils.nextRequestEnd();
      prefix = "\n\n";
      ...
      gremlin.editor.clearSelection();
      gremlin.editor.moveCursorTo(pos.row + prefix.length, 0);
   ...
   function init() {
      ...
      history_popup.on('shown', function () {
         ...
         history_viewer.getSession().setMode("ace/mode/gremlin");
//         history_viewer.setTheme("ace/theme/monokai");
   ...
   function saveCurrentEditorState(server, username, password, content) {
      var timestamp = new Date().getTime();
      localStorage.setItem("editor_state", JSON.stringify(
         { 'time': timestamp, 'server': server, 'username': username, 'password': password, 'content': content }));
